...
- [ ] **Goal 5: Fix Failing Test + Error Logging** — Fix `mail.test.ts` SMTP config test. Add `logger.warn` to webhook `.catch()` handlers. Add `logger.error` to AI suggestion route catch. Wrap `JSON.parse` in rbac.ts. Fix cron auth to use `timingSafeEqual`. (Small, Quality)
