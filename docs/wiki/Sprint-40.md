...
- [x] **Goal 8: Move use-proposal-form.ts to lib + add users.id defaultFn** — Move hook from `components/` to `lib/`. Add `$defaultFn(() => randomUUID())` to `users.id` in schema for consistency with all other tables.
