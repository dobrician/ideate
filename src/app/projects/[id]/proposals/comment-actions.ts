...
export async function addComment(
  ...
  return withActionAuth(formData.get("csrfToken") as string, {
    ...
    // Resolve projectId for permission + deadline checks — proposal comments need the parent project
    let deadlineProjectId = projectId;
    if (!deadlineProjectId && proposalId) {
    ...
    if (deadlineProjectId && !(await canActOnProject(user.role as Role, user.id, deadlineProjectId, "comment:create"))) {
