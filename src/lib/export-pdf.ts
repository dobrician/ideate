...
export async function generatePdf(
  ...
  const meta = `Status: ${project.status}  |  Deadline: ${formatDate(project.deadline, locale)}  |  Created: ${formatDate(project.createdAt, locale)}`;
  ...
  if (project.projectComments && project.projectComments.length > 0) {
    ...
    for (const c of project.projectComments) {
      ...
      doc.setFont("helvetica", "bold");
      checkPage(12);
      doc.text(`${c.authorName}  (${formatDate(c.createdAt, locale)})`, margin + 4, y);
      y += 4;
      doc.setFont("helvetica", "normal");
  ...
  for (const proposal of project.proposals) {
    ...
    doc.setTextColor(100);
    doc.text(
      `by ${proposal.authorName}  |  ${formatDate(proposal.createdAt, locale)}`,
      margin + 2,
      y
    );
    y += 5;
    ...
    if (proposal.comments.length > 0) {
      ...
      for (const comment of proposal.comments) {
        ...
        checkPage(10);
        doc.text(
          `${comment.authorName}  (${formatDate(comment.createdAt, locale)})`,
          margin + 6,
          y
        );
        y += 3;
  ...
  doc.text(
    `Generated by Ideate on ${new Date().toLocaleDateString(locale ?? "en-US", { year: "numeric", month: "long", day: "numeric" })}`,
    margin,
    y
  );
